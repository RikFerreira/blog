...
After installed, generate the `/etc/fstab` file with the `genfstab` command. Remember to pass the `-U` option for using the [UUID](https://en.wikipedia.org/wiki/Universally_unique_identifier) of the partitions. Don't worry, you still can edit the file later, just be sure that it will be OK before rebooting.
